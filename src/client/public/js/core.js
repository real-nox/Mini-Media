...
async function AddPostDOM(post) {
    ...
    let isfollowed = await HasFollowed(post.user_id)
    if (!post.username || post.username === "null")
        div += `<article id="post-${post.post_id}">
                    <div class="post">
                        <div class="user"> 
...
async function showList(list) {
    ...
    for (const element of list) {
        let isfollowed = await HasFollowed(element.user_id)
        if (!element.username || element.username === "null") continue
        div += `<article id="post-${element.post_id}">
                    <div class="post">
                        <div class="user"> 
                            <a href="/${element.username}"> ${element.username}</a>`
        if (element.user_id !== currentOwner)
            div += `<button class="follow_unfollow" data-id="${element.user_id}"> ${isfollowed ? "Unfollow" : "Follow"} </button>`
        else
            div += `<div class="post-actions">
        ...
        div += `<p> ${element.p_content} </p>
                        ...
                        <div class="bottomarticle">
                            <span><button class="like-btn" data-post-id="${element.post_id}" data-likes="${element.likes}">Like ${element.likes}</button></span>
